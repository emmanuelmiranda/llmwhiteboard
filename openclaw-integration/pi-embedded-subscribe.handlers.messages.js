...
export function handleMessageUpdate(ctx, evt) {
    ...
    // PATCH: Handle thinking events with parallel text content - emit directly
    if (evtType === "thinking_delta" || evtType === "thinking_end") {
        ...
        if (partial && Array.isArray(partial.content)) {
            ...
            if (textBlock && textBlock.text) {
                ...
                console.log("[PATCH] Emit assistant:", deltaText ? deltaText.substring(0, 50) : "NONE");
                if (deltaText) {
    ...
    const next = ctx
        ...
        .trim();
    console.log("[PATCH] next.length=", next ? next.length : 0, "deltaBuffer.length=", ctx.state.deltaBuffer.length);
    if (next) {
        ...
        console.log("[PATCH] shouldEmit=", shouldEmit, "deltaText.length=", deltaText ? deltaText.length : 0);
        if (shouldEmit) {
            emitAgentEvent({
                runId: ctx.params.runId,
                stream: "assistant",
...
export function handleMessageEnd(ctx, evt) {
    const msg = evt.message;
    console.log("[PATCH-END] handleMessageEnd called, msg.role:", msg?.role);
    if (msg?.role !== "assistant") {
        return;
    }
    ...
    console.log("[PATCH-END] rawText length:", rawText?.length, "rawText preview:", rawText?.substring(0, 100));
    appendRawStream({
        ts: Date.now(),
    ...
    console.log("[PATCH-END] stripped text length:", text?.length, "text preview:", text?.substring(0, 100));
    ...
    // Use rawText if text is empty (thinking-only responses)
    const textToEmit = text || rawText;
    if (textToEmit) {
        const prevText = ctx.state.lastStreamedAssistantCleaned || "";
        // Only emit if we have new content (avoid duplicate if already streamed)
        if (textToEmit !== prevText && textToEmit.length > prevText.length) {
            console.log("[PATCH-END] Emitting COMPLETE text:", textToEmit.length, "chars (prev:", prevText.length, ")");
            emitAgentEvent({
                runId: ctx.params.runId,
                stream: "assistant",
                data: {
                    text: textToEmit,
                    delta: textToEmit.slice(prevText.length),
                    final: true
                }
            });
        } else {
            console.log("[PATCH-END] Skipping emit: textToEmit === prevText or shorter");
        }
    } else {
        console.log("[PATCH-END] No text to emit (both text and rawText empty)");
    }
